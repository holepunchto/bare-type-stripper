 ...
 * @param input - The TypeScript source to strip, as a string or a `Buffer`.
 * @param encoding - Encoding used to decode `input` when it is a string (default `'utf8'`); ignored when `input` is already a `Buffer`.
 * @param opts - An options object; currently unused.
 * @throws {TypeError} `input` is neither a string nor a buffer.
 * @throws {SyntaxError} the source contains non-erasable TypeScript syntax (`enum`/`const enum`, `namespace`/`module` with a body, parameter properties, or angle-bracket type assertions).
 */
